    ...
    var JTablePlugin = function (context) {
        ...
        self.events = {
            'summernote.init': function (_, layoutInfo) {
                self.selectionMask = $('<div class="jtable-block"><div/>');
                layoutInfo.editingArea.append(self.selectionMask);
                ...
                layoutInfo.editingArea.on('mousemove', '.note-editable', function (event) {
                    if (!tableBlock.pressed) return true;
                    modules.tablePopover.hide();
                    $(this).closest('.note-editing-area').addClass('no-text-select');
                ...
                layoutInfo.editingArea.on('mouseup', '.note-editable', function (event) {
                    ...
                    $(this).closest('.note-editing-area').removeClass('no-text-select');
                    $(this).closest('.note-editable').prop('contenteditable', 'true');
                ...
                layoutInfo.editingArea.on('mousedown', 'td', function (event) {
                    ...
                    var rightFlag = false;
                    if (targetRight - 10 <= event.pageX) {
                        rightFlag = true;
                    }
                    var bottomFlag = false;
                    if (targetBottom - 10 <= event.pageY) {
                        bottomFlag = true;
                    }
            ...
            'summernote.mousedown': (we, e) => {
                console.log('111');
                if (!tableBlock.pressed && self.selectionMask) {
                    self.selectionMask.hide();
                }
                // if (this.update(e.target, e)) {
                //   e.preventDefault();
                // }
            },
            'summernote.keyup summernote.scroll summernote.change summernote.disable summernote.blur summernote.dialog.shown': () => {
